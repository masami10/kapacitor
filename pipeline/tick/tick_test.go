...
import (
	...
	"github.com/masami10/kapacitor/pipeline/tick"
)
...
func TestPipelineImplemented(t *testing.T) {
	tickPkg, err := importer.For("source", nil).Import("github.com/masami10/kapacitor/pipeline/tick")
	if err != nil {
		t.Fatalf("error importing github.com/masami10/kapacitor/pipeline: %v", err)
	}
	// tickScope lists all the types in the tick package
	tickScope := tickPkg.Scope()

	pipelinePkg, err := importer.For("source", nil).Import("github.com/masami10/kapacitor/pipeline")
	if err != nil {
		t.Fatalf("error importing github.com/masami10/kapacitor/pipeline: %v; perhaps kapacitor is not in $GOPATH/src/influxdata/kapacitor?", err)
	}
